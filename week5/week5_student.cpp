...
void keyboard_check(){
      ...
      ver = keyboard.sequence_num;
      heartbeat = keyboard.sequence_num;
      pitch_command = ((keyboard.pitch - 128) / 128.0) * 15;
      roll_command = ((keyboard.roll - 128) / 128.0) * 15;
      thrust_command = ((keyboard.thrust - 128) / 128.0) * thrust_gain + neutral_thrust; 
      neutral_power =  thrust_command;
        // int roll;
        // int yaw;
        // int thrust;
      ...
      if (command != 0 && (ver != prev_version)){
        ...
        if (command == 32){
          ...
          neutral_power = (PWM_MAX - 1000) / 2.0 + 1000;
          neutral_thrust = neutral_power;
        } else if (command == 35){
          calibrate_imu();
        }
      } 
...
double r_D = 115;
double r_I = 0.01;
double roll_error_prev = 0;
double p_e_i = 0;
double r_e_i = 0;
int MAX_I = 200;
void pid_update(){
  double pitch_error, roll_error;
  double p_e_p, p_e_d, r_e_p, r_e_d;
  int p_u, r_u;
  printf("commanded pitch %d\n", -pitch_command);
  printf("sensed pitch %f\n", pitch_smooth);
  fprintf(f,"%d,%f\n",-pitch_command, pitch_smooth);
  pitch_error = pitch_smooth - -pitch_command;
  roll_error = roll_smooth - roll_command;
  // pitch_error = -pitch_error;
  p_e_p = p_P * pitch_error;
  r_e_p = r_P * roll_error;
  p_e_d = p_D * (pitch_error - pitch_error_prev);
  r_e_d = r_D * (roll_error - roll_error_prev);
  p_e_i += pitch_error * p_I;
  r_e_i += roll_error * r_I;
  if (p_e_i >= MAX_I){
    p_e_i = MAX_I;
  } else if (p_e_i <= -MAX_I){
    p_e_i = -MAX_I;
  ...
  p_u = (int) p_e_p + p_e_d + p_e_i; //PID
  r_u = (int) r_e_p + r_e_d + r_e_i; //PID
  // u = (int) e_p ; //P
  // u = (int) e_d; //D
  // groups: 0 & 1, 2 & 3
  // printf("effort: %d\tangle: %f\n", u, pitch_smooth);
  set_PWM(3, neutral_power - p_u - r_u);
  set_PWM(2, neutral_power - p_u + r_u);
  set_PWM(1, neutral_power + p_u - r_u);
  set_PWM(0, neutral_power + p_u + r_u);
  pitch_error_prev = pitch_error;
  // fprintf(f, "%d,%d,%f,%f\n", (neutral_power + u),(neutral_power - u),pitch_smooth, pitch_accel);
}
...
{    
    ...
    setup_imu();
    init_pwm();
    //grab user input PDA values if they exist
     if (argc > 4){
      p_P = (double) atoi(argv[1]);
      p_I = (double) atof(argv[2]);
      p_D = (double) atoi(argv[3]);
      A = (double) atof(argv[4]);
      printf("using addition arg P:%f\tI:%f\tD:%f\tA:%f\n", p_P, p_I,p_D, A);
    } else if (argc > 1){
      p_P = (double) atoi(argv[1]);
      p_I = (double) atof(argv[2]);
      p_D = (double) atoi(argv[3]);
      printf("using addition arg P:%f\tI:%f\tD:%f\n", p_P, p_I, p_D);
    } 
    calibrate_imu();
 
    command = keyboard.key_press;
    prev_version = keyboard.sequence_num;
    prev_heartbeat = keyboard.sequence_num;
